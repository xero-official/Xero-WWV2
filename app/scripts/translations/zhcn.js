...
ONBOARD_interface_content__6    :  '如果你将 *公钥(地址)* 提供给别人, 那么对方就能给你发送XERO或者代币. 👍',
...
ONBOARD_secure_1_content__4     :  '正常情况下地址栏左侧会始终显示 `MYXEROERWALLET INC` 字样的绿色字体.',
...
TRANS_warning               : '如果你使用“只发送XERO”或则“只发送ETC”功能，那么你是通过合约发送交易。 一些服务（例如交易所充值）接收这些交易时会遇到问题。阅读更多内容。 ',
...
TRANSModal_Title            : '“只发送XERO”和“只发送ETC”交易 ',
TRANSModal_Content_0        : '注意不同的交易和不同的服务： ',
TRANSModal_Content_1        : '**发送XERO（标准交易）：** 这将生成一个默认交易，直接从一个地址发送到另一个地址。它的默认gas是21000。 用这种方式发送XERO可能会在ETC链上重放。 ',
TRANSModal_Content_2        : '**只发送：XERO：** 这是通过[Timon Rapp的重放保护合约（VB推荐）](https://blog.ethereum.org/2016/07/26/onward_from_the_hard_fork/)， 所以你只会在XERO链上发送交易。 ',
TRANSModal_Content_3        : '**只发送：ETC：** 这是通过[Timon Rapp的重放保护合约（VB推荐）](https://blog.ethereum.org/2016/07/26/onward_from_the_hard_fork/)， 所以你只会在XERO链上发送交易。 ',
TRANSModal_Content_4        : '**Coinbase & ShapeShift: ** 通过标准交易只发送XERO或者ETC。如果你通过“只发送XERO或者ETC”合约发送交易，你需要联系他们的客服，让他们手动增加你的余额或者返币给你。 [你也可以尝试Shapeshift的”分割“工具。](https://split.shapeshift.io/) ',
...
/* Geth Error Messages */
GXERO_InvalidSender          : '无效发送者 ',
GXERO_Nonce                  : 'Nonce太低 ',
GXERO_Cheap                  : 'Gas价格太低，不被接受 ',
GXERO_Balance                : '余额不足 ',
GXERO_NonExistentAccount     : '账户不存在或者账户余额太低 ',
GXERO_InsufficientFunds      : '余额不足，余额低于 gas * price + 发送数额 ',
GXERO_IntrinsicGas           : '固有gas太低 ',
GXERO_GasLimit               : '超过区块gas上限 ',
GXERO_NegativeValue          : '负值 ',
