...
TRANS_warning               : '"Only XERO" 或 "Only ETC" 功能是透過一個合約來傳送，並非單純的轉錢。有些服務可能沒辦法接受這兩種功能。 了解更多。Functions you are sending via a contract. Some services have issues accepting these transactions. Read more. ',
TRANS_advanced              : '進階功能：為交易新增Data',
TRANS_data                  : '交易的Data ',
TRANS_gas                   : 'Gas 總量 ',
TRANS_sendInfo              : '我們使用比平均手續費略高的gas價格（每單位0.000000021 XERO）確保交易能快速被確認。我們並不收取任何手續費。',
...
SWAP_order_CTA              : "請傳送 ", // Please send 1 XERO...
...
ERROR_17                    : '你帳戶中必須有至少 **0.01 XERO** 來支付gas的成本。 請增加一些 XERO 並重試。 ',
...
SUCCESS_1                   : '有效的地址 ',
SUCCESS_2                   : '成功解密錢包 ',
SUCCESS_3                   : '你的交易已被發布到網路上。這並不表示該交易已被確認或完成。遇到區塊鏈網路流量極大時，你可能需要超過三個小時才能完成交易 1) 在下面確認你的交易狀態。 2) 如果該交易已經等待(pending)數小時或消失，使用 檢查交易狀態 頁面來替換它。 3) 使用 [XERO加油站](https://ethgasstation.info/) 來確認最佳的Gas價格。 4) 存下該筆交易的雜湊值，你可能需要它:   ', //'Your TX has been broadcast to the network. This does not mean it has been mined & sent. During times of extreme volume, it may take 3+ hours to send. 1) Check your TX below. 2) If it is pending for hours or disappears, use the Check TX Status Page to replace. 3) Use [XERO Gas Station](https://ethgasstation.info/) to see what gas price is optimal. 4) Save your TX Hash in case you need it later:    ', //
SUCCESS_4                   : '成功新增你的錢包 ',
SUCCESS_5                   : '已選擇檔案 ',
SUCCESS_6                   : '你已經成功連上 ',
SUCCESS_7                   : '訊息簽章驗證成功',
...
/* Geth Error Messages */
GXERO_InvalidSender          : '無效的傳送者 ',
GXERO_Nonce                  : 'Nonce值太低 ',
GXERO_Cheap                  : 'Gas價格太低以致不會被礦工接受 ',
GXERO_Balance                : '餘額不足 ',
GXERO_NonExistentAccount     : '帳戶不存在或是帳戶餘額不足 ',
GXERO_InsufficientFunds      : '提供的金額不足以支付gas成本的錢加上你要轉的錢 ',
GXERO_IntrinsicGas           : '提供的gas量不足 ',
GXERO_GasLimit               : 'gas量超過每單位區塊的gas上限 ',
GXERO_NegativeValue          : '負數的值 ',
